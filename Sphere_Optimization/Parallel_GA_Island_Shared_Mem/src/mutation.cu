...
__device__
void mutation(curandState *d_state, double *islandPop, const int p, int tid, 
    ...
    for (unsigned int i = 0; i < p; i++) {
        ...
        if (myrand < mutationProbability) 
            islandPop[threadIdx.x * p + i] = lowerBound + (curand_uniform(&d_state[tid])) * (upperBound - lowerBound);
    }
}