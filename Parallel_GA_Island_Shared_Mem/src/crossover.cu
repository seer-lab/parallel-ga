    ...
    if (threadIdx.x < (individualsPerIsland/2)) {                        
        if (myrand < crossoverProbability) {
            for (unsigned int i = 0; i < p; i++) {
                ...
                islandPop[threadIdx.x * p + i] = 0.5*(((1+b) * islandParent[threadIdx.x * p + i]) + ((1-b) * islandParent[(threadIdx.x+(individualsPerIsland/2)) * p + i]));
                islandPop[(threadIdx.x+(individualsPerIsland/2)) * p + i] = 0.5*(((1-b) * islandParent[threadIdx.x * p + i]) + ((1+b) * islandParent[(threadIdx.x+(individualsPerIsland/2)) * p + i]));
            }
        } else {
            for (unsigned int i = 0; i < p; i++) {
                islandPop[threadIdx.x * p + i] = islandParent[threadIdx.x * p + i];
